...
def excel_takip_guncelle(al_listesi):
    ...
    # Kolon tiplerini pandas LossySetitemError vermeyecek şekilde esnek yap
    for c in col_names:
        if c not in df_veriler.columns:
            df_veriler[c] = np.nan

    # Tarih ve metin kolonlarını object/datetime tipine güvenle çevir
    df_veriler['Çıkış Tarihi'] = pd.to_datetime(df_veriler['Çıkış Tarihi'], errors='coerce')
    df_veriler['AL Tarihi'] = pd.to_datetime(df_veriler['AL Tarihi'], errors='coerce')
    df_veriler['Durum'] = df_veriler['Durum'].astype(str)
    df_veriler['Çıkış Nedeni'] = df_veriler['Çıkış Nedeni'].astype(str)
            ...
            if 'Tarih' in df_bist.columns and len(df_bist) > 0:
                ...
                if tarih_today not in df_bist['Tarih_str'].values:
                    new_bist_row = pd.DataFrame([{
                        'Tarih': pd.to_datetime(df_xu.index[-1]),
    ...
    for idx, row in df_veriler.iterrows():
        ...
        if durum == 'AÇIK':
            ...
            last_date = pd.to_datetime(df_h.index[-1])
            ...
            df_veriler.loc[idx, 'Güncel Fiyat'] = round(last_close, 2)
            df_veriler.loc[idx, 'En Yüksek Fiyat'] = round(new_high, 2)
            df_veriler.loc[idx, 'En Düşük Fiyat'] = round(new_low, 2)
            df_veriler.loc[idx, 'MFE (%)'] = round(mfe_pct, 2)
            df_veriler.loc[idx, 'MAE (%)'] = round(mae_pct, 2)
            df_veriler.loc[idx, 'Güncel Getiri (%)'] = round(guncel_getiri_pct, 2)
            df_veriler.loc[idx, 'Stop Fiyatı'] = round(active_stop, 2)

            try:
                al_tarihi = pd.to_datetime(row['AL Tarihi'])
                df_veriler.loc[idx, 'Pozisyonda Gün'] = max(1, (last_date - al_tarihi).days)
            except:
                pass
            ...
            if last_low <= active_stop:
                ...
                df_veriler.loc[idx, 'Durum'] = 'KAR STOP' if active_stop > giris_fiyati else 'STOP'
                df_veriler.loc[idx, 'Çıkış Tarihi'] = last_date
                df_veriler.loc[idx, 'Çıkış Fiyatı'] = round(cikis_fiyati, 2)
                df_veriler.loc[idx, 'Çıkış Nedeni'] = cikis_nedeni
                df_veriler.loc[idx, 'Gerçekleşen Getiri (%)'] = round(getiri, 2)
                df_veriler.loc[idx, 'Güncel Getiri (%)'] = np.nan
